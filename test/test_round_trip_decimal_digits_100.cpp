...
BOOST_AUTO_TEST_CASE(fixed_point_type_decimal_digits_100)
{
  ...
  for(count = UINT32_C(1); ((count < UINT32_C(100000)) && b); ++count)
  {
    ...
    const bool next_test_result =
      local::round_trip(local::fixed_point_type_decimal_digits_100(fx));

    b = (b && next_test_result);
  }

  BOOST_CHECK_EQUAL(count, UINT32_C(100000));
