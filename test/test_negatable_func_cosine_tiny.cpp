...
//! \file
//!\brief Tests for the trogonometric cosine function of (fixed_point) for a tiny digit range.

#include <cmath>

#define BOOST_TEST_MODULE test_negatable_func_cosine_tiny
#define BOOST_LIB_DIAGNOSTIC

#include <boost/fixed_point/fixed_point.hpp>
#include <boost/test/included/unit_test.hpp>

BOOST_AUTO_TEST_CASE(test_negatable_func_cosine_tiny)
{
  typedef boost::fixed_point::negatable<4, -11> fixed_point_type;
  typedef fixed_point_type::float_type          float_point_type;

  const fixed_point_type tol = ldexp(fixed_point_type(1), fixed_point_type::resolution + 5);
