...
#define BOOST_TEST_MODULE test_negatable_basic_metal_config_32bit
...
BOOST_AUTO_TEST_CASE(test_negatable_basic_metal_config_32bit)
{
  ...
  const fixed_point_type d =
    local::first_derivative(fixed_point_type(1) / 2U,  // x
                            fixed_point_type(1) / 8U,  // Step size dx.
  ...
  // We obtain a fixed-point result of approximately 4.6, which
  // can not be differentiated from the exact value.
