...
namespace local
{
  ...
  fixed_point_type_decimal_digits_002;

  bool round_trip(const fixed_point_type_decimal_digits_002& x);
}

bool local::round_trip(const local::fixed_point_type_decimal_digits_002& x)
{
  typedef local::fixed_point_type_decimal_digits_002 fixed_point_type;
...
BOOST_AUTO_TEST_CASE(test_negatable_round_trip_digits10_002)
{
  ...
  for(count = UINT16_C(1); ((count < number_of_test_cases) && b); ++count)
  {
    ...
    const bool next_test_result = local::round_trip(local::fixed_point_type_decimal_digits_002(x));
