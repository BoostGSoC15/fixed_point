...
//! \brief Example program showing a bare metal real-time performance measurement of exp(negatable), 32-bit.
...
numeric_type x = numeric_type(7) / 10;
numeric_type y;
...
void app::benchmark::task_func()
{
  ...
  // exp(7/10) = approx. 2.013752707470477
  const bool value_is_ok =    (y > (numeric_type(20) / 10))
                           && (y < (numeric_type(21) / 10));
