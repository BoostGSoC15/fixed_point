...
int main()
{
  typedef boost::multiprecision::number<boost::multiprecision::backends::cpp_bin_float<963>> float_point_type;

  const std::uintmax_t float_point_resolution_digits10(std::numeric_limits<float_point_type>::digits10);

  const float_point_type a( float_point_type(2U) / 3U);
  const float_point_type b( float_point_type(4U) / 3U);
  const float_point_type z(-float_point_type(3U) / 4U);

  const local::stopwatch<std::chrono::high_resolution_clock> timer;

  const float_point_type h = hypergeometric_1f1(a, b, z);

  const float elapsed = timer.elapsed<float>();

  std::cout << std::setprecision(std::streamsize(float_point_resolution_digits10))
            << h
            << std::endl
            << "Run time:     "
            << std::setprecision(3)
            << elapsed << " sec, "
            << " showing " << std::streamsize(float_point_resolution_digits10) << " decimal digits."
            << std::endl;
}
